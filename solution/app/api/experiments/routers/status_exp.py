...
from app.database.models import Experiment, ExperimentStatus, UserRole, ExperimentApproval
...
router = APIRouter(tags=["Experiment Status"])
...
async def change_experiment_status(
    ...
    current_user = Depends(check_permissions([UserRole.ADMIN, UserRole.APPROVER, UserRole.EXPERIMENTER]))
):
    stmt = select(Experiment).where(Experiment.id == experiment_id).options(
        joinedload(Experiment.creator), joinedload(Experiment.approvals)
    )
    ...
    if new_status == ExperimentStatus.APPROVED:
        ...
        if not (await session.execute(vote_stmt)).scalar_one_or_none():
            session.add(ExperimentApproval(experiment_id=experiment_id, approver_id=current_user.id))
            await session.flush()

        votes_count = await session.scalar(select(func.count(ExperimentApproval.id)).where(ExperimentApproval.experiment_id == experiment_id))
