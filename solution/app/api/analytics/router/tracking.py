...
async def track_event(
    ...
    exposure_stmt = select(Exposure).where(
        Exposure.decision_id == data.decision_id
    )
    exposure = (await session.execute(exposure_stmt)).scalar_one_or_none()

    if not exposure:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail={
                "message": "Невалидный decision_id",
                "hint": "Экспозиция с таким ID не найдена"
            }
        )
    ...
    await GuardrailService.check_and_trigger(
        session,
...
async def get_report(
    ...
    current_user=Depends(
        check_permissions([UserRole.ADMIN, UserRole.EXPERIMENTER])
    )
):
